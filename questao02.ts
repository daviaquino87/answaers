...
function validateSuffix(telephone:string):void{
  let suffix = telephone.split("@");

  if(suffix[1] == "whatsapp.net"){
    PrivateValidNumber++
    listValidNumber.push(suffix[0])
  } 

  if(suffix[1] == "g.us"){
    validGroups++;
    listValidGroups.push(suffix[0])
  }
}

arrayList.forEach(telephone => {
  totalProcess++;
  if(validateNumber(telephone)){
    validateSuffix(telephone)
  }
})

let response =`
valid private totals: ${PrivateValidNumber}
valid private list: ${listValidNumber}
total valid groups: ${validGroups}
list valid groups: ${listValidGroups}
total processed: ${totalProcess}
`
